...
#define CMD_ENTER_OTA_MODE 0xA0 
...
static esp_gatts_attr_db_t secure_svc_db[SECURE_IDX_NB] = {
    [SECURE_IDX_SERVICE] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&primary_service_uuid, ESP_GATT_PERM_READ, sizeof(uint16_t), sizeof(uint16_t), (uint8_t *)&GATTS_SERVICE_UUID}},

    [SECURE_IDX_CHAR_A] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&char_decl_uuid, ESP_GATT_PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&prop_read_notify}},

    [SECURE_IDX_CHAR_VAL_A] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_A, ESP_GATT_PERM_READ, sizeof(char_a_value), sizeof(char_a_value), char_a_value}},

    [SECURE_IDX_CHAR_CFG_A] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&char_client_cfg_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, sizeof(uint16_t), 0, NULL}},

    [SECURE_IDX_CHAR_B] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&char_decl_uuid, ESP_GATT_PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&prop_read}},

    [SECURE_IDX_CHAR_VAL_B] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_B, ESP_GATT_PERM_READ, sizeof(char_b_value), sizeof(char_b_value), char_b_value}},

    [SECURE_IDX_CHAR_C] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&char_decl_uuid, ESP_GATT_PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&prop_read}},

    [SECURE_IDX_CHAR_VAL_C] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_C, ESP_GATT_PERM_READ, sizeof(char_c_value), sizeof(char_c_value), char_c_value}},
};

static esp_gatts_attr_db_t battery_svc_db[BATTERY_IDX_NB] = {
    [BATTERY_IDX_SERVICE] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&primary_service_uuid, ESP_GATT_PERM_READ, sizeof(uint16_t), sizeof(uint16_t), (uint8_t *)&BATTERY_SERVICE_UUID}},

    [BATTERY_IDX_CHAR] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&char_decl_uuid, ESP_GATT_PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&prop_read}},

    [BATTERY_IDX_VAL] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&BATTERY_LEVEL_UUID, ESP_GATT_PERM_READ, sizeof(uint8_t), sizeof(uint8_t), &battery_level}},

    [BATTERY_IDX_CFG] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&client_characteristic_configuration_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, sizeof(uint16_t), sizeof(uint16_t), (uint8_t *)battery_ccc}},
};

static esp_gatts_attr_db_t command_svc_db[COMMAND_IDX_NB] = {
    [COMMAND_IDX_SERVICE] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&primary_service_uuid, ESP_GATT_PERM_READ, sizeof(uint16_t), sizeof(uint16_t), (uint8_t *)&COMMAND_SERVICE_UUID}},

    [COMMAND_IDX_CHAR] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&char_decl_uuid, ESP_GATT_PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&prop_rw}},

    [COMMAND_IDX_VAL] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&COMMAND_CHAR_UUID, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, sizeof(command_value), sizeof(command_value), command_value}},
...
if (param->write.len == 1 && param->write.value[0] == CMD_ENTER_OTA_MODE)
    {
      ESP_LOGI(TAG, "Received command to enter OTA mode");
      // Trigger OTA mode
      trigger_ota_mode();
    }
    else if (param->write.len == 1 && param->write.value[0] == CMD_ENTER_OTA_MODE)
    {
      // Trigger OTA mode
      trigger_ota_mode();
    }
    else if (param->write.handle == secure_handle_table[SECURE_IDX_CHAR_CFG_A])
    {
      // Handle CCCD write for secure service notifications
      ccc_default = *(uint16_t *)param->write.value;
      ESP_LOGI(TAG, "Secure Service CCCD updated: 0x%04X", ccc_default);
    }
    else if (param->write.handle == battery_handle_table[BATTERY_IDX_VAL])
    {
      // Update battery level
      battery_level = *(uint8_t *)param->write.value;
      ESP_LOGI(TAG, "Battery level updated: %d%%", battery_level);
      esp_ble_gatts_set_attr_value(battery_handle_table[BATTERY_IDX_VAL], sizeof(battery_level), &battery_level);
    }
